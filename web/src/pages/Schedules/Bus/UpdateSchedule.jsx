...
import { getBusJourneyById, updateScheduleJourney } from '../../../services/busJourneyService';
import { getUsers } from '../../../api/user';
import { getAllBusRoutes } from '../../../services/busRouteService';
import { getAllBuses } from '../../../services/busService';
import { getAllEmployees } from '../../../services/employeeService';

const BusScheduleUpdate = () => {
    const { id } = useParams();
    ...
    useEffect(() => {
        ...
        // Fetch the existing bus journey data by ID for updating
        getBusJourneyById(id).then((busJourney) => {
            const { route, departureTime, arrivalTime, bus, driver, boardedUsers, state } = busJourney;
            setFormData({
                route: route._id,
                departureTime,
                arrivalTime,
                bus: bus._id,
                driver: driver._id,
                boardedUsers: boardedUsers.map((user) => user._id),
                state,
            });
        });
    }, [id]);
    ...
    const handleSubmit = async () => {
        await updateScheduleJourney(id, formData);
    };
    ...
    return (
        <div className="my-16">
            <PageHeader title="Update Bus Schedule" isHiddenButton={true} />
...
export default BusScheduleUpdate;
